...
Be specific and objective. Use English for the description as it works best with image generation models.`,
  ...
  // ────────────────────────────────────────────────────────
  // ════════════════════════════════════════════════════════════════
  // جلب السيناريوهات الجاهزة (تعمل بدون LLM)
  // ════════════════════════════════════════════════════════════════
  getPrebuiltScripts: publicProcedure.query(async () => {
    const { PREBUILT_SCRIPTS } = await import('./prebuiltScripts');
    return PREBUILT_SCRIPTS.map(s => ({
      id: s.id,
      title: s.titleAr,
  ...
  // إنتاج من سيناريو جاهز (بدون LLM)
  produceFromPrebuilt: publicProcedure
    .input(z.object({
      scriptId: z.string(),
    ...
    .mutation(async ({ input }) => {
      const { PREBUILT_SCRIPTS, prebuiltToVideoScript } = await import('./prebuiltScripts');
      const prebuilt = PREBUILT_SCRIPTS.find(s => s.id === input.scriptId);
      if (!prebuilt) throw new Error(`سيناريو غير موجود: ${input.scriptId}`);

      const script = prebuiltToVideoScript(prebuilt, input.customTitle);
      const jobId = `prebuilt_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

      await createJobInDB(jobId, script.title, script, input.options ?? {});

      const qUserId = `anon_${Math.random().toString(36).slice(2, 8)}`;
      jobQueue.enqueue(jobId, qUserId, () => runProductionJob(jobId, script, input.options ?? {}))
        .then(async ({ position, waitSeconds }) => {
          if (position > 0) {
            await updateJobInDB(jobId, {
              status: 'pending',
      ...
      return { jobId, title: script.title, sceneCount: script.scenes.length };
    }),

  // إنتاج من سيناريو يدوي كامل (بدون LLM)
  produceFromManual: publicProcedure
    .input(z.object({
      ...
      scenes: z.array(z.object({
        ...
        narration: z.string().min(5).max(1000),
        duration: z.number().min(3).max(15).default(6),
      })).min(1).max(10),
      options: productionOptionsSchema.optional(),
    }))
    .mutation(async ({ input }) => {
      const { buildManualScript } = await import('./prebuiltScripts');
      const script = buildManualScript(input.title, input.language, input.voice, input.scenes);
      const jobId = `manual_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

      await createJobInDB(jobId, input.title, script, input.options ?? {});
      ...
      return { jobId, title: input.title, sceneCount: input.scenes.length };
    }),

  // اقتراح سيناريو جاهز حسب الوصف (بدون LLM)
  suggestPrebuilt: publicProcedure
    .input(z.object({
      description: z.string().min(2),
      category: z.string().optional(),
    }))
    .query(async ({ input }) => {
      const { findBestScript, PREBUILT_SCRIPTS } = await import('./prebuiltScripts');
      const best = findBestScript(input.description, input.category);
      // إرجاع 3 اقتراحات: الأفضل + اثنان عشوائيين
      const others = PREBUILT_SCRIPTS
        .filter(s => s.id !== best.id)
        .sort(() => Math.random() - 0.5)
        .slice(0, 2);
      return [
        { id: best.id, title: best.titleAr, description: best.description, category: best.category, emoji: best.emoji, tags: best.tags, sceneCount: best.scenes.length, isBestMatch: true },
        ...others.map(s => ({ id: s.id, title: s.titleAr, description: s.description, category: s.category, emoji: s.emoji, tags: s.tags, sceneCount: s.scenes.length, isBestMatch: false })),
      ];
    }),
